...
void createBricks(){
    ...
    for(int i=0;i<brickAmount;i++){
        glColor3d(0,0,255);
        if ((i<90&&i>=80)||(i<20&&i>=10))
            glColor3d(255,0,0);
        else if ((i<80&&i>=70)||(i<30&&i>=20))
            glColor3d(0,255,0);
        else if ((i<70&&i>=60)||(i<40&&i>=30))
            glColor3d(30,0,120);
        else if ((i<60&&i>=50)||(i<50&&i>=40))
            glColor3d(0,40,150);
...
void print(int a){
    ...
    for(int i = 0; i < chance.length(); i++){
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, chance[i]);
    }
    ...
    for(int i = 0; i < prevScore.length(); i++){
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, prevScore[i]);
    }
...
glutCreateWindow("Game Project OpenGL");
